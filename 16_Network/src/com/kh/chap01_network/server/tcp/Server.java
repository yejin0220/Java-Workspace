...
public class Server {
	 ...
	 * 5. 클라리언트 정보를 통해서 outputStream 생성
	 * 6. 클라이언트 정보를 통해서 inputStream 생성
	 * 7. 스트림을 통해 입력 및 출력
	 * 8. 통신종료
	 * - inputStream, OutputStream, Socket 을 전부 닫아줘야함(close))
	 * 
	 * 
	 */
	...
	public void serverStart() {
		...
		try {
			...
			while(true)	 {
			//3. 접속요청이 오면 요청 수락(accept())후 해당 클라이언트에 대한 소켓 객체 생성
			//4. 클라이언트의 정보를 저장
			...
			//클라이언트로부터 전달된 값을 한ㄴ줄단위로 읽어들이기 위한 입력용 스트림
			BufferedReader br = new BufferedReader(new InputStreamReader(is));
			
			//클라이언트에게 값을 한 줄단위로 출력하는 스트림
			PrintWriter pw = new PrintWriter(os, true);//데이터를 출력할때 print(), println() 메소드를 사용
			
			//7. 스트림을 통해 읽고 쓰기
			while(true) {
				String message = br.readLine(); //클라이언트로 부터 입력이 있기 전까지 멈춤
				...
				if(message == null || "exit".equals(message)) {
					...
					pw.flush(); //현재 스트림에 있는 데이터를 강제로 내보내는 메서드 close()메소드 호출되면 자동으로 flush()메소드 호출됨	
					}
