...
public class Run {
	...
	public static void main(String[] args) {
		...
		Thread t1 = new Thread(new Car()); //start메소드 호출이 car메서드 실행
		...
		//메모리가 모자랄때 우선순위를 정할 수가 있음
		//스레드의 우선순위를 정하지 않으면 OS(운영체제)가 임의로 수행시킨다.
		System.out.println("===========우선순위 변경 전============");
		System.out.println(Thread.currentThread().getName()+"의 우선순위"+Thread.currentThread().getPriority());//현재실행중인 스레드의 우선순위
		...
		//()안에 숫자를 넣어줘도 되지만, 의미부여해 필드로 지정해주는게 좋음
		t1.setPriority(Thread.MIN_PRIORITY); //()안에는 우선순위를 지정해주면 됨 => 상수필드로 중요도 가장 작은 수 지정 : 1
		t3.setPriority(Thread.MAX_PRIORITY); //							 => 상수필드로 중요도 가장 높은 수 지정 : 10
		 ...
		 * 해결책 : start() 시키기 전에 우선순위, 주종관게 를 설정해야함
		 * 
		 * 주종관계 설정
		...
		//시작 순서는 우선순위와 다를 수 있지만, 종료순서는 우선순위(priority)가 높은 순으로 끝나게 됨(항상 그렇지는 않음)
