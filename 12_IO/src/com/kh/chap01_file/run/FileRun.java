...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//2.  경로지정을 한 파일명(C:\\aaa\\test.txt)을 제시해서 생성해야함.(c드라이브에aaa파일 생성해둠)
			File file2 = new File("C:\\aaa\\test.txt");
			file2.createNewFile();
			
			//3. 폴더를 생성하고나서 그 안에 파일을 생성하는 방법
			File bbbFolder = new File("C:\\bbb"); //c드라이브에 bbb파일이 생성됨
		 ...
		 * 		보조 스트림 : 기반스트림만으로는 부족한 성능을 향샹시켜주는 용도의 스트림(직접적으로 외부매체와 연결할 수 없음, 즉 단독사용이 불가)
